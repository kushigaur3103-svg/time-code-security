...
ENTERPRISE_DEVSECOPS_SYSTEM_PROMPT = (
    ...
    "RULE 5 (CWE-78 COMMAND EXECUTION - ZERO TAINT):\n"
    "When generating command execution code:\n"
    "1. For dynamic host/network targets: NEVER pass raw user variables into command lists. You MUST sanitize with 'import shlex' and 'shlex.quote(param)' first:\n"
    "   safe_host = shlex.quote(user_input)\n"
    "   ping_cmd = ['ping', '-c', '1', safe_host]\n"
    "   subprocess.run(ping_cmd, check=False)\n\n"
    "2. For file/directory inspections (e.g. ls): NEVER include untrusted user input in the command arguments. Use hardcoded safe base paths only:\n"
        ...
        try:
            ...
            if not fixed_code:
                standalone_prompt = (
                    ...
                    "1. CWE-78 Command Execution (Zero Taint): STRICT BAN on .replace() character blacklists.\n"
                    ...
                    "     ping_cmd = ['ping', '-c', '1', safe_host]\n"
                    "     subprocess.run(ping_cmd, check=False)\n"
                    "   - For file/directory inspections (e.g. ls): NEVER include untrusted user input in the command arguments. Use hardcoded safe base paths only:\n"
                    "     ls_cmd = ['ls', '-la', '/var/data']\n"
                    "     subprocess.run(ls_cmd, check=False)\n"
