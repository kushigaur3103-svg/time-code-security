    ...
    return ft.Container(
        content=ft.Column(
            [
                ...
                ft.Container(height=4),
                ft.Text(
                    f"VERIFIED SECURITY BASELINE ({len(ALL_46_CWES)} CWES PASSED):",
                    size=10,
                    weight=ft.FontWeight.BOLD,
                    color="#6b7280",
                ),
...
def main(page: ft.Page):
    ...
    def show_dashboard():
        ...
        btn_view_rules = ft.ElevatedButton(
            f"Rules Catalog ({len(ALL_46_CWES)} CWEs)",
            icon=ICON_SECURITY,
            bgcolor="#111827",
            color="#9ca3af",
            style=ft.ButtonStyle(
        ...
        cwe_filter_dropdown = ft.Dropdown(
            options=[
                ft.dropdown.Option("ALL", f"All CWEs ({len(ALL_46_CWES)} Rules)"),
                *[
                    ft.dropdown.Option(cwe, f"{cwe}: {name}")
                    for cwe, name in ALL_46_CWES
                ]
            ],
            value="ALL",
            width=360,
